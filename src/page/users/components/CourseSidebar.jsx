...
const CourseSidebar = ({ onDataFetched, onVideoSelect, onEnrollmentStatus }) => {
  ...
  if (!courseData) return <p>Không có dữ liệu khóa học.</p>;

  return (
    <div className="w-1/4 h-screen m-2 overflow-y-auto">
      <div className="max-w-5xl mx-auto p-6 space-y-8">
        {courseData.sections.map((section, sectionIndex) => (
          <div
            key={section.id}
            className="bg-white border-l-4 border-indigo-600 rounded-xl shadow-md p-5"
          >
            ...
            <ul className="space-y-4">
              {section.lessons.map((lesson) => {
                ...
                return (
                  <li
                    key={lesson.id}
                    className="p-4 bg-gray-50 border border-gray-200 rounded-lg hover:shadow transition duration-300"
                  >
                    <h3 className="text-lg font-semibold text-indigo-700">{lesson.title}</h3>
                    <p className="text-sm text-gray-500 mb-2">⏱ Thời lượng: {lesson.duration}</p>
                      ...
                      {isFirstSection || isEnrolled ? (
                        ...
                        <>
                          <span className="inline-block text-sm text-gray-400">
                            ▶️ Xem video <span className="ml-1">🔒</span>
                          </span>
                          <br />
                          <span className="inline-block text-sm text-gray-400">
                            📄 Xem tài liệu <span className="ml-1">🔒</span>
                          </span>
                          <p className="text-sm text-red-600">Vui lòng đăng ký khóa học để truy cập.</p>
                        </>
                      )}
                    </div>
                  </li>
