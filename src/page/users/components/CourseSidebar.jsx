...
const CourseSidebar = ({ onDataFetched, onVideoSelect, onEnrollmentStatus }) => {
  ...
  if (loading) return <p className="text-base sm:text-lg text-gray-600">Đang tải...</p>;
  if (error) return <p className="text-base sm:text-lg text-gray-600">Lỗi: {error}</p>;
  if (!courseData) return <p className="text-base sm:text-lg text-gray-600">Không có dữ liệu khóa học.</p>;

  return (
    <div className="w-full md:w-1/4 h-auto md:h-screen m-0 sm:m-1 overflow-y-auto md:overflow-y-auto">
